*resources/views/pmik/pelaporan_sensus.blade.php*
<x-app-layout>
<style>
...
/* FOLDER CARD - horizontal/wide card */
.folder-card {
    background: #fff;
    border-radius: 12px;
    padding: 14px 20px;
    ...
    <div class="main">
        <div class="navbar">
            @include('layouts.navbar', [
                'title' => '📂 Arsip Sensus Bulanan',
                'description' => 'Laporan sensus pasien tersusun berdasarkan bulan dan tahun.'
            ])
        </div>

        <!-- Header Bar -->
        <div class="sensus-header-bar">
            <a href="{{ route('pmik.pelaporan') }}" class="btn-back">
        ...
        <!-- Arsip Folder Per Tahun (default: collapsed) -->
        ...
        <div class="archive-section" data-year="{{ $tahun }}">
            ...
            <div class="folder-grid collapsed" id="year-{{ $tahun }}">
                ...
                <a href="{{ route('pmik.pelaporan.sensus.detail', ['month' => str_pad($item['bulan'], 2, '0', STR_PAD_LEFT), 'year' => $tahun]) }}" class="folder-card" data-month="{{ $item['bulan'] }}" data-month-name="{{ $item['bulan_nama'] }}">
        ...
        <div class="empty-panel">
            ...
            <span class="text">Belum ada data sensus yang tersedia.</span>
        </div>
        @endforelse
    </div>
</div>
...
function applyFilter() {
    ...
    let bulanNum = '';
    if (bulanInput) {
        const bulanOptions = document.querySelectorAll('#bulanList option');
        bulanOptions.forEach(opt => {
            if (opt.value.toLowerCase() === bulanInput) {
                bulanNum = opt.getAttribute('data-num');
            }
        });
    }
    ...
    document.querySelectorAll('.archive-section').forEach(section => {
        ...
        if (tahunInput && sectionYear !== tahunInput) {
            section.classList.add('hidden');
        } else {
            section.classList.remove('hidden');

            // Auto expand matching year
    ...
    if (bulanNum) {
        document.querySelectorAll('.folder-card').forEach(card => {
            const parentSection = card.closest('.archive-section');
            if (parentSection.classList.contains('hidden')) return;

            const cardMonth = card.getAttribute('data-month');
            if (cardMonth !== bulanNum) {
