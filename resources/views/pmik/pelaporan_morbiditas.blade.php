*resources/views/pmik/pelaporan_morbiditas.blade.php*
<x-app-layout>
<style>
...
.year-badge { background:linear-gradient(135deg, #10B981, #059669); color:#fff; font-size:18px; font-weight:800; padding:8px 20px; border-radius:10px; box-shadow:0 3px 10px rgba(16,185,129,0.3); }
.year-line { flex:1; height:2px; background:linear-gradient(90deg, #A7F3D0, transparent); border-radius:2px; }
.year-toggle { font-size:18px; color:#10B981; transition:transform 0.3s ease; }
...
.folder-card:hover { border-color:#A7F3D0; background:#F0FDF4; box-shadow:0 4px 14px rgba(16,185,129,0.1); transform:translateX(4px); }
...
.folder-count .dot { width:6px; height:6px; border-radius:50%; background:#10B981; }
.folder-arrow { color:#9CA3AF; font-size:16px; transition:color 0.2s ease; }
.folder-card:hover .folder-arrow { color:#10B981; }
    ...
    <div class="main">
        <div class="navbar">
            @include('layouts.navbar', [
                'title' => '📂 Arsip 10 Besar Morbiditas',
                'description' => 'Laporan morbiditas tersusun berdasarkan bulan dan tahun.'
            ])
        </div>
        ...
        <div class="archive-section" data-year="{{ $tahun }}">
            ...
            <div class="folder-grid collapsed" id="year-{{ $tahun }}">
                ...
                <a href="{{ route('pmik.pelaporan.morbiditas.detail', ['month' => str_pad($item['bulan'], 2, '0', STR_PAD_LEFT), 'year' => $tahun]) }}" class="folder-card" data-month="{{ $item['bulan'] }}">
        ...
        <div class="empty-panel">
            ...
            <span class="text">Belum ada data morbiditas yang tersedia.</span>
        </div>
        @endforelse
    </div>
</div>
